...
class SelectServiceScreen extends StatefulWidget {
  ...
  SelectServiceScreen({this.categoryId, this.subCategoryId, this.isUpdate = false, this.packageData, this.providerId});
...
class _SelectServiceScreenState extends State<SelectServiceScreen> {
  ...
  Future<void> fetchAllServices({String? searchText = "", int? categoryId = -1, int? subCategoryId = -1}) async {
    ...
    await getServicesList(page, search: searchText, categoryId: categoryId, subCategoryId: subCategoryId, providerId: widget.providerId, type: SERVICE_TYPE_FIXED).then((value) {
      isApiCalled = true;

      if (page == 1) serviceList.clear();

      isLastPage = value.serviceList!.length != PER_PAGE_ITEM;

      serviceList.addAll(value.serviceList!);
  ...
  Future<void> getCategory() async {
    ...
    await getCategoryList(perPage: CATEGORY_TYPE_ALL, categoryList: categoryList).then((value) async {
      categoryList = value.validate();
      if (widget.isUpdate && categoryId != -1) selectedCategory = categoryList.firstWhere((element) => element.id == widget.categoryId);
  ...
  Future<void> getSubCategory({required int categoryId}) async {
    getSubCategoryList(catId: categoryId.toInt(), subCategoryList: subCategoryList).then((value) async {
      subCategoryList = value.validate();
      CategoryData allValue = CategoryData(id: -1, name: "All");
      subCategoryList.insert(0, allValue);

      if (widget.isUpdate) {
        if (subCategoryId != -1) {
          selectedSubCategory = subCategoryList.firstWhere((element) => element.id == subCategoryId);
        }
      } else {
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('${locale.categoryBasedPackage} ${appStore.isCategoryWisePackageService ? locale.enable : locale.disable}', style: boldTextStyle(size: LABEL_TEXT_SIZE)),
                      4.height,
                      Text(locale.categoryBasedPackageEnableMessage, style: secondaryTextStyle()),
                    ],
                  ).expand(),
                  Transform.scale(
                    scale: 0.8,
                    child: Switch.adaptive(
                      ...
                      onChanged: (v) {
                        showConfirmDialogCustom(
                          ...
                          title: '${locale.doYouWantTo} ${!appStore.isCategoryWisePackageService ? locale.enable : locale.disable} ${locale.categoryBasedPackage}?',
                ...
                Column(
                  children: [
                    DropdownButtonFormField<CategoryData>(
                      decoration: inputDecoration(context, fillColor: context.cardColor, hint: locale.selectCategory),
                    ...
                    16.height,
                    DropdownButtonFormField<CategoryData>(
                      decoration: inputDecoration(context, fillColor: context.cardColor, hint: locale.selectSubcategory),
              ...
              if (!isPackageTypeSingle.validate())
                AppTextField(
                  ...
                  decoration: inputDecoration(context, hint: locale.searchHere),
                  onFieldSubmitted: (s) {
                    appStore.setLoading(true);
              ...
              Column(
                ...
                children: [
                  16.height,
                  Text(locale.includedInThisPackage, style: boldTextStyle(size: LABEL_TEXT_SIZE)).paddingSymmetric(horizontal: 16),
                  16.height,
                  if (!appStore.isLoading && appStore.selectedServiceList.isNotEmpty)
                    ...
                    Container(
                      ...
                      child: Text(locale.packageServicesWillAppearHere, style: secondaryTextStyle()).center(),
                    ),
                  16.height,
                ],
              ),
              ...
              Column(
                ...
                children: [
                  ...
                  Text(locale.showingFixPriceServicesMessage, style: secondaryTextStyle()),
                  8.height,
                ],
              ).paddingSymmetric(horizontal: 16),
              //
                    ...
                    return Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          16.width,
                          Icon(isSelected ? Icons.check_circle : Icons.radio_button_unchecked, size: 28, color: isSelected ? context.primaryColor : context.iconColor),
                          8.width,
                        ],
                      ).onTap(
                        () {
